...
class TestFullPushSyncCycle:
    ...
    def test_push_and_sync(self, tmp_path: Path) -> None:
        ...
        settings = Settings(data_dir=str(tmp_path), upstream=BASE)
        ...
        with respx.mock:
            ...
            respx.head(f"{BASE}/v2/myapp/blobs/{config_digest}").mock(
                return_value=Response(404)
            )
            respx.head(f"{BASE}/v2/myapp/blobs/{layer_digest}").mock(
                return_value=Response(404)
            )
            ...
            respx.post(f"{BASE}/v2/myapp/blobs/uploads/").mock(
                side_effect=[
                    Response(
                        202, headers={"Location": f"{BASE}/v2/myapp/blobs/uploads/u1"}
                    ),
                    Response(
                        202, headers={"Location": f"{BASE}/v2/myapp/blobs/uploads/u2"}
                    ),
                ]
            )
            ...
            respx.put(f"{BASE}/v2/myapp/blobs/uploads/u2").mock(
                return_value=Response(201)
            )
            # PUT manifest
...
class TestPullProxyFromUpstream:
    ...
    def test_pull_manifest_from_upstream(self, tmp_path: Path) -> None:
        ...
        with respx.mock:
            respx.get(f"{BASE}/v2/remote-app/manifests/latest").mock(
